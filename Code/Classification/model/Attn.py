...
class WordAttention(nn.Module):
    ...
    def forward(self, input_sequences, pos_input_sequences, dep_input_sequences):
            ...
            #print(input_sequences.view(-1,input_sequences.size(2)).shape)
            #print('hhaha')
            #print(self.embedding.weight.shape)
            #print(self.embedding(input_sequences).shape)
            embeddings = torch.mm(input_sequences.view(-1,input_sequences.size(2)), self.embedding.weight)
            #print(embeddings.shape)
            embeddings = embeddings.view(input_sequences.size(0), input_sequences.size(1), self.embedding.weight.shape[1])
            #print(embeddings.shape)
